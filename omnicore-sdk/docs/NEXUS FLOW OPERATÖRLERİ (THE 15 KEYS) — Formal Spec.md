...
| **12**  | `?;`      | Halt (Son)        | Akışı durdurur; yerel masalar ve kaynaklar imha eder. dosyaları kapatır, bellek free yapar |
...
* **(masa):** Masalar sadece adres tutarlar, adresleri taşırlar ve iletirler. Masa ile dosya açılmış (v.b işlemler) ise aynı isimde bir değişkene aktarılır, ama sadece adresi taşınır verinin kendisi değil.
...
* Başarılı ise `(h)` masasına değerin adresi yakalanır ve akış normal devam eder.
...
(in, out) -> {
    v:data <- file.read(in);
    file.write(out, data);
}?; # Akış biter, masalar imha edilir,açık dosya kapatılır, tutulan bellek de free olur..
```
