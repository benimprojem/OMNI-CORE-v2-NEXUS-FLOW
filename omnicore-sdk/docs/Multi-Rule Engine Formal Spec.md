...
```nexus
r:[w > 2800] :=> "Genişlik sınırı aşıldı";
r:[h > 2100] :=> "Yükseklik sınırı aşıldı";
```
...
rules PrimeBorders(n) {
    r: n < 2  :=> "Sınır İhlali: 2'den küçük asallık olmaz"; 
    r: n == 2 -> Success;
    r: n == 3 -> Success;
    
    r: ( (n + 1) % 6 == 0 || (n - 1) % 6 == 0 ) :=> "Asal Değil: Tünel Dışı";
